...
def compute_pc_limits(pc_rc, T_shape, fallback_vmin=None, fallback_vmax=None):
    ...
    vmax_ms, linthresh_ms : float
        Symmetric limit and linear threshold for SymLogNorm (mean-subtracted movie).
    """
    ...
    vmax_ms = np.percentile(np.abs(ms_all), 99)
    linthresh_ms = np.percentile(abs_ms, 10) if len(abs_ms) > 0 else vmax_ms * 0.01

    return vmin_pc, vmax_pc, vmax_ms, linthresh_ms


def _render_pc_movie_pair(args):
    """
    Top-level worker for parallel PC movie rendering. Renders both the LogNorm and
    mean-subtracted SymLogNorm movies for one PC group.

    Must be a top-level function (not a method or closure) to be picklable by
    multiprocessing.Pool. All args are plain Python/numpy objects.

    Args tuple
    ----------
    pc_rc         : ndarray (n_channels, n_times)
    pcs           : list of int
    T, J          : ndarray  — theta/J meshgrids from MakeAnimations
    jphi_c        : ndarray  — J_phi bin centers
    times         : ndarray  — simulation times
    channel_name  : str
    time_ratio    : float    — simulation unit to Gyr conversion
    file_directory: str
    vmin_pc, vmax_pc         : LogNorm limits
    vmax_ms, linthresh_ms    : SymLogNorm limits
    cmap_log_name : str      — colormap name for LogNorm movie
    cmap_sym_name : str      — colormap name for SymLogNorm movie
    """
    import matplotlib
    matplotlib.use('Agg')
    matplotlib.rcParams['text.usetex'] = False  # avoid luatex subprocess per frame
    import matplotlib.pyplot as plt
    import matplotlib as mpl
    ...
    (pc_rc, pcs, T, J, jphi_c, times, channel_name, time_ratio,
     file_directory, vmin_pc, vmax_pc, vmax_ms, linthresh_ms,
     cmap_log_name, cmap_sym_name) = args

    cmap_log = mpl.cm.get_cmap(cmap_log_name)
    cmap_sym = mpl.cm.get_cmap(cmap_sym_name)

    if len(pcs) == 1:
        pc_string = 'pc{}'.format(pcs[0])
    elif np.any(np.diff(pcs) > 1):
        pc_string = 'pc{}'.format(pcs)
    else:
        pc_string = 'pc{}-{}'.format(pcs[0], pcs[-1])

    def _animate_frame(timestep, ax, subtract_mean, norm_function, cmap, **kwargs):
        ax.clear()
        if 'pitch' in channel_name:
            dat = -np.reshape(pc_rc[:, timestep], T.shape, 'F')
        else:
            dat = np.reshape(pc_rc[:, timestep], T.shape, 'F')
        if subtract_mean:
            dat -= np.mean(dat, axis=0)
        ax.pcolormesh(T, J, dat, cmap=cmap, norm=norm_function(**kwargs))
        ax.set_yticks([np.min(jphi_c), np.max(jphi_c)],
                      labels=[r'$J_\phi=1000$', r'$J_\phi=4000$'],
                      fontsize=18, color='k')
        ax.set_rmax(np.max(jphi_c))
        ax.tick_params(left=False, right=False, labelleft=True,
                       labelbottom=False, bottom=False)
        ax.text(3 * np.pi / 2, 100,
                r'{} Gyr'.format(np.around((times[0] + timestep) * time_ratio, 2)),
                fontsize=20, ha='center', c='k')
        ax.grid(False)
        ax.set_rlabel_position(50)
        title = '{}, {}{}'.format(channel_name, pc_string,
                                  ' (mean-subtracted)' if subtract_mean else '')
        ax.set_title(title, fontsize=15)
        plt.draw()

    movie_configs = [
        (False, mpl.colors.LogNorm, cmap_log,
         {'vmin': vmin_pc, 'vmax': vmax_pc}, ''),
        (True, mpl.colors.SymLogNorm, cmap_sym,
         {'vmin': -vmax_ms, 'vmax': vmax_ms, 'linthresh': linthresh_ms},
         '_mean_subtracted'),
    ]

    for subtract_mean, norm_function, cmap, norm_kwargs, suffix in movie_configs:
        fname = file_directory + pc_string + suffix + '.mp4'
        fig, ax = plt.subplots(1, 1, figsize=(7, 6), subplot_kw={'projection': 'polar'})
        ax.pcolormesh(T, J, np.ones(T.shape), cmap=cmap, norm=norm_function(**norm_kwargs))
        cbar = plt.colorbar(ax.collections[0], ax=ax)
        cbar.set_label(channel_name, fontsize=20)
        fig.tight_layout()
        anim = FuncAnimation(
            fig,
            partial(_animate_frame, ax=ax, subtract_mean=subtract_mean,
                    norm_function=norm_function, cmap=cmap, **norm_kwargs),
            frames=np.arange(0, len(times), 1),
            interval=20,
            blit=False,
        )
        anim.save(fname, writer=animation.FFMpegWriter(fps=10))
        plt.close()
...
def make_face_on_movies(mssa, data_file, times, face_on_dir, list_of_pc_lists,
    ...
    cmap_sym       : colormap for mean-subtracted SymLogNorm movies (default cmr.prinsenvlag_r)
    """
    import cmasher as cmr
    if cmap_log is None:
        cmap_log = cmr.sunburst
    if cmap_sym is None:
        cmap_sym = cmr.prinsenvlag_r
